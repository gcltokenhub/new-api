...
- **视觉基线（强制）**：新增与修改 UI 前阅读 [Token工厂前端视觉与交互规范](../docs/design/frontend-visual-guidelines.md)，默认延续已确认的 Figma 门户风格，通过主题 token 和现有组件落实蓝绿配色、清晰字阶、统一间距与轻量卡片。
- **场景适配**：门户采用宽布局与重点渐变；控制台、管理页保持表格和表单的信息密度，保留暗色模式及用户主题设置。已有页面在修改范围内逐步统一，偏离规范需说明原因。
- **品牌与入口**：站点名称优先原样使用后台“系统名称”设置，“Token工厂”仅为空配置默认值，不得重新增加硬编码的 New-API 品牌展示及仓库、文档、社区、反馈和上游版本更新入口。
...
- **2026-09-16**：将 Figma 门户视觉风格确立为全前端新增和修改 UI 的默认规范，补充场景适配与视觉验收要求。
